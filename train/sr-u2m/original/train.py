...
class S2UT(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        src_mel[src_mel == self.hparams.pad_index] = 0.0
        enc_out = self.modules.embedding_model(src_mel)
        # calculate mellogits
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        mel, mel_lens = batch.tgt_mel
        mel[mel == self.hparams.pad_index] = 0
        ids = batch.id
        # speech translation loss
        loss = self.hparams.mel_cost(pred_mel, mel, mel_lens)
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides("tgt_sig","tgt_mel")
