...
class S2UT(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        src_mel_fore, _ = batch.src_mel_fore

        code, code_lens = batch.code

        # Use default padding value for wav2vec2
        src_mel_fore[src_mel_fore == self.hparams.pad_index] = 0.0
        enc_out = self.modules.embedding_model(src_mel_fore)
        # calculate mellogits
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        src_mel_back, src_mel_back_lens = batch.src_mel_back
        src_mel_back[src_mel_back == self.hparams.pad_index] = 0
        ids = batch.id
        # speech translation loss
        loss = self.hparams.mel_cost(pred_mel, src_mel_back, src_mel_back_lens)
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides("src_mel_fore","src_mel_back")
    def src_audio_pipeline(wav):
        ...
        src_mel_fore = hparams["mel_spectogram"](audio=audio_fore).transpose(0,1)
        src_mel_back = hparams["mel_spectogram"](audio=audio_back).transpose(0,1)
        
        return src_mel_fore, src_mel_back
